...
export function getFrameResultsGlobalFrame(
  ...
  if (beamOnBeamConnections) {
    ...
    for (const conn of beamOnBeamConnections) {
      for (const secBeamId of conn.secondaryBeamIds) {
        ...
        // Secondary beam must not get automatic releases from beam-on-beam classification
        ...
        if (frameEndReleases) {
          ...
          if (rel) {
            ...
            const ni = isReversed ? rel.nodeJ : rel.nodeI;
            const nj = isReversed ? rel.nodeI : rel.nodeJ;
            relI = { Ux: ni.ux, Uy: ni.uy, Uz: ni.uz, Rx: ni.rx, Ry: ni.ry, Rz: ni.rz };
            relJ = { Ux: nj.ux, Uy: nj.uy, Uz: nj.uz, Rx: nj.rx, Ry: nj.ry, Rz: nj.rz };
          }
        }
  ...
  // Build beam release lookup from explicit input-tab releases only
