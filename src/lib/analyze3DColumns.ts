...
export function getFrameResults3D(
  ...
  // ── Helper: keep centre-to-centre span, but sample end moments at faces ──
  function sampleMomentsAtBeamFaces(
    Lcc: number,
    ...
    if (!stations || stations.length < 2 || halfColLeft + halfColRight < 1e-6) {
      return { span: Lcc, Mleft, Mright, Mmid, stations };
    }
    ...
    const faceLeft = sampleAt(halfColLeft);
    const faceRight = sampleAt(Math.max(halfColLeft, Lcc - halfColRight));
    const faceStations = [...stations];
    faceStations[0] = faceLeft;
    faceStations[faceStations.length - 1] = faceRight;
    return { span: Lcc, Mleft: faceLeft, Mright: faceRight, Mmid, stations: faceStations };
  }
  ...
  return frames.map((frame): FrameResult => {
    ...
    for (const beamId of frame.beamIds) {
      ...
      const trimmed = sampleMomentsAtBeamFaces(
